...
const Dashboard = () => {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      const monthlyRevenue = items.reduce((sum, item) => sum + (item.value || 0), 0);

      setStats({
        totalViews: totalViews > 0 ? totalViews : Math.floor(Math.random() * 10000) + 5000,
        activeListings,
        completedTrades,
        savedItems: Math.floor(Math.random() * 50) + 20,
        monthlyRevenue: monthlyRevenue > 0 ? monthlyRevenue : Math.floor(Math.random() * 5000) + 2000,
        responseRate: 98,
        avgRating: 4.8,
  ...
  const featuredItems = allItems.slice(0, 6).map((item, index) => ({
    ...
    originalPrice: item.value ? `₹${Math.floor(item.value * 1.3).toLocaleString()}` : "",
    discountPercent: Math.floor(Math.random() * 30) + 10,
    image: item.images?.[0]?.url || `https://images.unsplash.com/photo-${1600000000000 + index}?w=400&h=300&fit=crop`,
    condition: item.condition,
    location: item.location || "Location not specified",
    timeAgo: new Date(item.createdAt?.toDate?.() || item.createdAt).toLocaleDateString(),
    ...
    features: ["Original", "Good Condition", "Well Maintained", "Fast Response"],
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div className="mb-8">
                  <div className="flex items-center justify-between mb-6">
                    <h2 className="text-xl font-bold text-marketplace-text-primary flex items-center">
                      <TrendingUp className="mr-2 text-marketplace-primary" size={24} />
                      Featured Items
                    </h2>
                    <Link
                      to="/browse"
                        ...
                        <div className="relative aspect-[4/3] overflow-hidden">
                          ...
                          <div className="absolute top-3 right-3 flex space-x-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                            <button className="w-8 h-8 bg-white/90 backdrop-blur-sm rounded-full flex items-center justify-center shadow-marketplace hover:bg-white transition-colors">
                              <Heart size={14} className="text-marketplace-gray-600 hover:text-marketplace-error" />
                            </button>
                          </div>
                        </div>
            ...
            <div className="lg:col-span-3 space-y-6">
              ...
              <div className="bg-white dark:bg-marketplace-gray-900 rounded-marketplace-lg border border-marketplace-border-light shadow-marketplace p-6">
                ...
                <div className="space-y-4">
                  {userItems.slice(0, 3).map((item, index) => (
                    <div key={item.id} className="flex items-center space-x-3 p-3 hover:bg-marketplace-gray-50 dark:hover:bg-marketplace-gray-800 rounded-marketplace transition-colors">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-marketplace-text-secondary">
                          Listed • {new Date(item.createdAt?.toDate?.() || item.createdAt).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                  ))}
                  ...
                  {userTrades.slice(0, 2).map((trade, index) => (
                    <div key={trade.id} className="flex items-center space-x-3 p-3 hover:bg-marketplace-gray-50 dark:hover:bg-marketplace-gray-800 rounded-marketplace transition-colors">
                      <div className="w-10 h-10 bg-marketplace-secondary bg-opacity-10 rounded-marketplace flex items-center justify-center">
                        <MessageCircle size={16} className="text-marketplace-secondary" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium text-marketplace-text-primary truncate">
                        ...
                        <p className="text-xs text-marketplace-text-secondary">
                          {trade.status} • {new Date(trade.createdAt?.toDate?.() || trade.createdAt).toLocaleDateString()}
                        </p>
                      </div>
                    </div>
                  ))}
              ...
              <div className="bg-white dark:bg-marketplace-gray-900 rounded-marketplace-lg border border-marketplace-border-light shadow-marketplace p-6">
                <h3 className="font-bold text-marketplace-text-primary mb-4 flex items-center">
                  <BarChart3 className="mr-2 text-marketplace-primary" size={20} />
                  Your Performance
                </h3>
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-marketplace-text-secondary">Response Rate</span>
                    <span className="text-sm font-semibold text-marketplace-success">{stats.responseRate}%</span>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-marketplace-text-secondary">Avg. Rating</span>
                    <div className="flex items-center space-x-1">
                      <Star size={12} className="text-marketplace-secondary fill-current" />
                      <span className="text-sm font-semibold text-marketplace-text-primary">{stats.avgRating}</span>
                    </div>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-marketplace-text-secondary">Total Messages</span>
                    <span className="text-sm font-semibold text-marketplace-text-primary">{stats.totalMessages}</span>
                  </div>
                </div>
              </div>
            </div>
